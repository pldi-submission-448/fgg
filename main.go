...
import (
	...
	"github.com/pldi-submission-448/fgg/internal/frontend"
)
...
func init() {
	...
	flag.BoolVar(&dictpass, "dictpass", false,
		"compile FGG source to dict pass in FG with syntax extensions")
	flag.BoolVar(&dictpassnoassertion, "dictpassnoassertion", false,
		"compile FGG source to dict pass in FG with syntax extensions")

	flag.BoolVar(&dynamic, "dynamic", false, "compile FGG source to dynamic in FG with syntax extensions")
...
var usage = func() {
	...
	fcgg [options] -fg  path/to/file.fg
	fcgg [options] -fgg path/to/file.fgg
	fcgg [options] -internal
	fcgg [options] -inline "package main; type ...; func main() { ... }"
...
func main() {
	...
	case interpFGG:
		...
		if (dictpass) {
			intrp_fgg.DictPass(false)
		} else if (dictpassnoassertion) {
			intrp_fgg.DictPass(true)
		}else if dynamic {
			intrp_fgg.Dyn()
		}

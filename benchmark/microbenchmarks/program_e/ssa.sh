...
ssa_dyn() {
  ./ssadump -build=F tmp_dyn.go |wc -l
}
...
echo $PARAM_M "$(get_LOC_new monom)" "$(get_LOC_new dict)" "$(get_LOC_new dict_no_assertion)" "$(get_LOC_new dyn)"
done
exit
...
~/go/bin/fgg -fgg -dictpass -- ./tmp.fgg > tmp_dict.go
~/go/bin/fgg -fgg -monomc -- ./tmp.fgg > tmp_monom.go
echo $PARAM_C "$(word_count)" "$(ssa_dictpass)" "$(ssa_monom)"
done
...
for PARAM_C in $(seq 2 5 200)
do
python3 ./make_fcgg_input_ordinary_call_chain.py --p=$PARAM_P --m=$PARAM_M --n=$PARAM_N --c="$PARAM_C" -chanops 1 > tmp.fgg
~/go/bin/fgg -fgg -dictpass -- ./tmp.fgg > tmp_dict.go
~/go/bin/fgg -fgg -monomc -- ./tmp.fgg > tmp_monom.go
echo $PARAM_C "$(word_count)" "$(ssa_dictpass)" "$(ssa_monom)"
done
...
~/go/bin/fgg -fgg -dictpass -- ./tmp.fgg > tmp_dict.go
~/go/bin/fgg -fgg -monomc -- ./tmp.fgg > tmp_monom.go
echo $PARAM_N "$(word_count)" "$(ssa_dictpass)" "$(ssa_monom)"
done

...
func (this DictPassTranslator) TranslateExpr(e fgg.FGGExpr, ds []fgg.Decl, delta fgg.Delta, eta map[fgg.TParam]fg.FGExpr, gamma fgg.Gamma) (expr fg.FGExpr, isStructLit bool) {
	...
	case fgg.StructLit: //I-VALUE
		...
		//new_es = append(new_es, MakeDictForLists(ds, eta, delta, e_.U_S.UArgs, tdecl.GetBigPsi())...)
		ret := fg.NewStructLit(fg.Type(AuxBracket(e_.U_S.GetName())), new_es)
		//this.env.fgExprIsStructLit[&ret] = true
		return ret, true
	case fgg.Select: //I-field
		e_ty := e_.GetExpr().Typing(ds, delta, gamma, false) //TODO: add a compile error report here
	...
	case fgg.Assert: //I-ASSERT
		...
		return ret, false
	case fgg.Call:
		callee_e_ty := e_.E_recv.Typing(ds, delta, gamma, false)
		new_e_bar := []fg.FGExpr{}
		var m_sig fgg.Sig
		md, _ := fgg.MethodsDelta(ds, delta, callee_e_ty)
		...
		//var _small_psi_dagger []fg.FGExpr
		//_small_psi_dagger = MakeDictForLists(ds, eta, delta, e_.GetTArgs(), m_sig.GetPsi())
		...
		case fgg.TParam: //I-DICTCALL
			...
			var ret fg.Call
			ty := delta[callee_e_ty_].(fgg.TNamed)
			e_assert := fg.NewAssert(e_dagger, fg.Type(AuxBracket(ty.TName)))

...
import (
	"fmt"
	"github.com/pldi-submission-448/fgg/internal/fgg/dictpass"
	"github.com/pldi-submission-448/fgg/internal/fgg/dyn"
	"io/ioutil"
	"os"
	"reflect"
	"strconv"
	...
	"github.com/pldi-submission-448/fgg/internal/fgr"
...
func (intrp *FGGInterp) DictPass(noassertion bool) {
	p_fgg := intrp.GetSource().(fgg.FGGProgram)
	p_mono := dictpass.TranslateToDictPassFG(p_fgg, noassertion)
	fmt.Println(p_mono.String())
}

func (intrp *FGGInterp) Dyn() {
	p_fgg := intrp.GetSource().(fgg.FGGProgram)
	p_mono := dyn.TranslateToDynFG(p_fgg)
	fmt.Println(p_mono.String())
}
...
func (intrp *FGGInterp) Monom(monom bool, compile string) {
	...
	if true {
		for _, decl := range p_mono.GetDecls() {
			if decl_, ok := decl.(fg.MethDecl); ok {
				dictpass.UnusedVarRemover{}.VisitMethodDecl(decl_)
			}
		}
	}
	if monom {
...
func renameParamsSTypeLit(c fgg.STypeLit) fgg.STypeLit {
	tfs_orig := c.Psi.GetTFormals()
	orig := c.GetFieldDecls()
	psi_sub := makeParamIndexAsDoubleBetaSubs(c.Psi)
	tfs := make([]fgg.TFormal, len(tfs_orig))
	for i, v := range tfs_orig {
		tfs[i] = fgg.NewTFormal(v.GetTParam().TSubs(psi_sub).(fgg.TParam), v.GetUpperBound().TSubs(psi_sub))
	}
	Psi_recv := fgg.NewBigPsi(tfs)

	fds := make([]fgg.FieldDecl, len(orig))
	for i, s1 := range orig {
		fds[i] = s1.Subs(psi_sub)
	}

	return fgg.NewSTypeLit(c.GetName(), Psi_recv, fds)
}
...
func renameParamsMethDecl(m fgg.MethDecl) fgg.MethDecl {
	...
	return fgg.NewMethDecl(m.GetRecvName(), m.GetRecvTypeName(), m.Psi_recv, m.GetName(), Psi_meth, pds, m.GetReturn().TSubs(subs), m.GetMethBody().TSubs(subs).(fgg.MethBody))
}

func renameReceiverTyParamsMethDecl(m fgg.MethDecl) fgg.MethDecl {
	subs := makeParamIndexAsDoubleBetaSubs(m.Psi_recv)
	tfs_orig := m.Psi_recv.GetTFormals()
	tfs := make([]fgg.TFormal, len(tfs_orig))
	for i, v := range tfs_orig {
		tfs[i] = fgg.NewTFormal(v.GetTParam().TSubs(subs).(fgg.TParam), v.GetUpperBound().TSubs(subs))
	}
	...
	return fgg.NewMethDecl(m.GetRecvName(), m.GetRecvTypeName(), Psi_recv, m.GetName(), m.Psi_meth, pds, m.GetReturn().TSubs(subs), m.GetMethBody().TSubs(subs).(fgg.MethBody))
}
...
func makeParamIndexAsDoubleBetaSubs(Psi fgg.BigPsi) fgg.Delta {
	subs := make(fgg.Delta)
	tfs := Psi.GetTFormals()
	for j := 0; j < len(tfs); j++ {
		subs[tfs[j].GetTParam()] = fgg.TParam("ββ" + strconv.Itoa(j+1))
	}
	return subs
}

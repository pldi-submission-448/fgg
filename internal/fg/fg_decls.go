...
import (
	...
	"github.com/pldi-submission-448/fgg/internal/base"
)
...
// TODO: NewMethDecl
func NewMDecl(recv ParamDecl, m Name, pds []ParamDecl, t TypeBase, e FGExpr, stmt []FGStmt) MethDecl {
	return MethDecl{recv: recv, name: m, pDecls: pds, t_ret: t, e_body: e, StmtBody: MethBody{stmt}}
}
func NewFieldDecl(f Name, t TypeBase) FieldDecl      { return FieldDecl{f, t} }
func NewParamDecl(x Name, t TypeBase) ParamDecl      { return ParamDecl{x, t} } // For fgg_monom.MakeWMap
func NewSig(m Name, pds []ParamDecl, t TypeBase) Sig { return Sig{m, pds, t} }  // For fgg_monom.MakeWMap
...
func (s STypeLit) Ok(ds []Decl) {
	...
	if isRecursiveFieldType(ds, make(map[TypeBase]TypeBase), s.t_S) {
...
func (f FieldDecl) GetType() TypeBase { return f.t }
...
func (md FuncType) String() string {
	var b strings.Builder
	b.WriteString("func ")
	b.WriteString("(")
	writeParamDecls(&b, md.ParamDecls)
	b.WriteString(") ")
	b.WriteString(md.RetType.String())
	return b.String()
}

//extended syntax for func signatures for dict pass.
//Should only be used for constructing source code
...
//implement FGExpr
func (fd FuncDecl) IsValue() bool {
	panic("implement me")
}

func (fd FuncDecl) CanEval(ds []base.Decl) bool {
	panic("implement me")
}

func (fd FuncDecl) ToGoString(ds []base.Decl) string {
	panic("implement me")
}

func (fd FuncDecl) Subs(subs map[Variable]FGExpr) FGExpr {
	panic("implement me")
}

func (fd FuncDecl) Typing(ds []Decl, gamma Gamma, allowStupid bool) TypeBase {
	panic("implement me")
}

func (fd FuncDecl) Eval(ds []Decl) (FGExpr, string) {
	panic("implement me")
}

func (md FuncDecl) String() string {
	var b strings.Builder
	b.WriteString("func ")
	b.WriteString("(")
	writeParamDecls(&b, md.ParamDecls)
	...
	b.WriteString(md.BodyExpr.String())
	b.WriteString(" }")
	return b.String()
}
...
func (o OtherDecls) String() string {
	return o.Def
}

func (o OtherDecls) GetName() base.Name {
	return o.name
}

func (o OtherDecls) Ok(ds []base.Decl) {
...
func (md MethDecl) GetReturn() TypeBase        { return md.t_ret }
...
func (md MethDecl) Ok(ds []Decl) {
	...
	allowStupid := false
	md.StmtBody.Typing(ds, env, allowStupid)
...
func (md MethDecl) String() string {
	...
	b.WriteString(" { ")
	b.WriteString(md.StmtBody.String())
	b.WriteString(" }")
	return b.String()
}
...
func (pd ParamDecl) GetType() TypeBase { return pd.t }
...
func (g Sig) GetReturn() TypeBase        { return g.t_ret }

func (g0 Sig) Ok(ds []Decl) {
	seen := make(map[TypeBase]ParamDecl)
...
func isTypeOk(ds []Decl, t TypeBase) bool { // Cf. IsStructType, etc.
	if t == STRING_TYPE {
		return true
	}
	if ch_ty, ok := t.(ChannelType); ok {
		return isTypeOk(ds, ch_ty.ElementType)
	}
	for _, v := range ds {
...
// Pre: isStruct(ds, t_S)
func isRecursiveFieldType(ds []Decl, seen map[TypeBase]TypeBase, t_S TypeBase) bool {
	if _, ok := seen[t_S]; ok {
		return true
	}
	for _, v := range fields(ds, t_S) {
		...
		seen1 := make(map[TypeBase]TypeBase)
		for k, v := range seen {
			seen1[k] = v
		}
		seen1[t_S] = t_S
